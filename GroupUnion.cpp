...
UnionNode *GroupUnion::FindHead(int group_id)
{  // the number of col that id in
...
void GroupUnion::UnionRecords(int groupRecord1, int groupRecord2)
{
    ...
    if (first->m_GroupID == second->m_GroupID)
    { // means that the two groups had been merged before
        return;
    }
